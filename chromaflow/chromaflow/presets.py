...
PRESETS: List[Preset] = [
    ...
    # -- programmable: each of these leans on a technique worth learning from --
    Preset("Pulsegrid", pal="Rust",
           fx="sin(y*3.0 + t*0.25)*0.20 + sin(x*2.0 - t*0.18)*0.15",
           fy="sin(x*3.0 - t*0.25)*0.20 + sin(y*2.0 + t*0.18)*0.15",
           wx="tri(s*(6.0 + a0*18.0) + t*0.2)*0.75",
           wy="tri(s*(6.0 + a0*18.0) + t*0.2 + 0.25)*0.75",
           wi="0.30 + mid*1.4",
           params={"decay": 0.960, "spin": 0.0, "zoom": 0.0, "lw": 2.0, "pcount": 2}),

    Preset("Solar flare", pal="Solar",
           fx="x/max(r,0.03)*(0.20 + bass*1.2)",
           fy="y/max(r,0.03)*(0.20 + bass*1.2)",
           wx="cos(s*tau*5 + t*0.4)*(0.15 + spec(s)*0.9)",
           wy="sin(s*tau*5 + t*0.4)*(0.15 + spec(s)*0.9)",
           wi="0.5 + spec(s)*2.2",
           params={"decay": 0.966, "spin": 0.02, "zoom": 0.01, "lw": 2.2, "pcount": 5,
                   "psize": 3.2, "bloom": 1.7, "aberr": 0.4, "palScale": 1.3}),

    Preset("Glass tunnel", pal="Mono",
           fx="-y*0.4 - x*(0.25 + treb*0.6)",
           fy=" x*0.4 - y*(0.25 + treb*0.6)",
           wx="cos(s*tau)*(0.10 + s*0.85)",
           wy="sin(s*tau)*(0.10 + s*0.85)",
           wi="0.55 + level*1.6",
           params={"decay": 0.975, "spin": 0.25, "zoom": -0.03, "lw": 1.6, "pcount": 0,
                   "vig": 0.7, "grain": 0.15, "palScale": 1.6, "bloom": 1.2}),

    Preset("Kelp forest", pal="Chloro",
           fx="noise(x*0.9 + t*0.12, y*0.5)*0.5*(0.3 + a1)",
           fy="0.35 + noise(x*0.6, y*0.9 - t*0.10)*0.4",
           wx="sin(s*tau*3 + t*0.3)*(0.08 + a1*0.5)",
           wy="s*2.0 - 1.0",
           wi="0.35 + mid*1.3",
           params={"spin": 0.0, "zoom": 0.0, "decay": 0.965, "lw": 2.4, "pcount": 3,
                   "grain": 0.2}),

    Preset("Radiant bloom", pal="Vapor",
           fx="-y*0.4 + noise(x*1.4, y*1.4 + t*0.15)*0.5",
           fy=" x*0.4 + noise(y*1.4, x*1.4 - t*0.15)*0.5",
           wx="cos(s*tau*4 + t*0.25)*(0.2 + 0.5*abs(sin(s*tau*2)))",
           wy="sin(s*tau*4 + t*0.25)*(0.2 + 0.5*abs(sin(s*tau*2)))",
           wi="0.4 + treb*1.4",
           params={"mirror": 6, "decay": 0.968, "spin": 0.06, "bloom": 1.4, "aberr": 0.3}),

    Preset("Deep current", pal="Ultra",
           fx="noise(x*0.7 + t*0.05, y*0.7)*0.6",
           fy="noise(y*0.7 - t*0.05, x*0.7)*0.6 + bass*0.2",
           wx="s*2.0 - 1.0",
           wy="noise(s*2.5, t*0.2)*0.45",
           wi="0.30 + level*1.1",
           params={"spin": 0.0, "zoom": 0.0, "decay": 0.978, "lw": 2.6, "pcount": 4,
                   "bloom": 0.6, "grain": 0.12, "vig": 0.6}),

    Preset("Strobe lattice", pal="Ember",
           fx="x*(0.3 + bass*2.2)",
           fy="y*(0.3 + bass*2.2) + sin(t*3.0)*0.1",
           wx="pulse(s*12.0 - t*0.5, 0.5)*0.6",
           wy="saw(s*12.0 + t*0.4)*0.6",
           wi="0.7 + mag*2.0",
           params={"decay": 0.945, "spin": 0.0, "zoom": 0.0, "lw": 2.4, "pcount": 4, "psize": 2.4,
                   "bloom": 1.5, "aberr": 0.7, "kick": 1.1}),

    Preset("Orbit lines", pal="Ion",
           fx="-y*0.30*(1.0 + a0*0.6)",
           fy=" x*0.30*(1.0 + a0*0.6)",
           wx="cos(s*tau*2 + t*0.3)*(0.3 + 0.4*sin(s*tau + t*0.5))",
           wy="sin(s*tau*2 + t*0.3)*(0.3 + 0.4*sin(s*tau + t*0.5))",
           wi="0.30 + bpm/240.0 + treb*0.8",
           params={"decay": 0.966, "spin": 0.0, "zoom": 0.0, "lw": 2.0, "pcount": 3}),
]
