...
class MultiMorphApp:
    ...
    def create_video(self):
        ...
        # Check audio duration if audio is provided
        audio_duration = None
        if self.audio_path:
            try:
                self.status_label.config(text="Checking audio duration...")
                audio_clip = AudioFileClip(self.audio_path)
                audio_duration = audio_clip.duration
                audio_clip.close()
                self.status_label.config(text=f"Audio duration: {audio_duration:.2f} seconds")
            except Exception as e:
                self.status_label.config(text=f"Warning: Could not check audio duration: {str(e)}")
        ...
        # Calculate total frames based on audio duration if available
        if audio_duration:
            # Calculate total frames based on audio duration
            total_frames = int(self.fps * audio_duration)
            
            # Calculate new duration per transition to match audio length
            self.duration_per_transition = audio_duration / (len(images) - 1)
            
            # Ensure each transition has the correct number of frames
            total_frames_per_transition = int(self.duration_per_transition * self.fps)
            
            self.status_label.config(text=f"Adjusted transition time to {self.duration_per_transition:.2f} seconds to match audio duration of {audio_duration:.2f} seconds")
        else:
            # Use default settings if no audio
        ...
        if self.audio_path:
            ...
            try:
                ...
                self.status_label.config(text=f"Video duration: {video_clip.duration:.2f}s, Audio duration: {audio_clip.duration:.2f}s")
                
                # Add audio to video - simpler approach
                self.status_label.config(text="Applying audio to video...")
                final_clip = video_clip.set_audio(audio_clip)
                
                # Write the result with basic settings
                self.status_label.config(text="Writing final video with audio...")
                ...
                # Close all clips to ensure proper resource cleanup
                self.status_label.config(text="Finalizing video...")
                final_clip.close()
                video_clip.close()
                audio_clip.close()
                ...
                self.status_label.config(text=f"Video created successfully with audio: {self.output_path}")
            except Exception as e:
                self.status_label.config(text=f"Error adding audio: {str(e)}")
                # If audio fails, just use the temp file
                if os.path.exists(temp_output):
                    os.rename(temp_output, self.output_path)
                    self.status_label.config(text=f"Created video without audio: {self.output_path}")
        else:
    ...
    def delaunay_morph(self, img1, img2, alpha):
        ...
        for triangle in tri.simplices:
            ...
            # Ensure triangles are np.float32 and have exactly 3 points with 2D coordinates
            warp_mat1 = cv2.getAffineTransform(np.float32(tri1_rect[:3]), np.float32(tri_morphed_rect[:3]))
            warp_mat2 = cv2.getAffineTransform(np.float32(tri2_rect[:3]), np.float32(tri_morphed_rect[:3]))
    ...
    def grid_warp(self, img1, img2, alpha):
        ...
        for channel in range(2):  # x and y channels
            ...
            if channel == 0:
                ...
                map_y2 = interp_vals
        # Apply the mapping
        # Convert map_x1, map_y1, map_x2, map_y2 to np.float32 as required by cv2.remap()
    ...
    def frequency_domain_morph(self, img1, img2, alpha):
        ...
        fft_img1 = fftshift(fft2(img1_gray))
        fft_img2 = fftshift(fft2(img2_gray))
        
        # Interpolate magnitude and phase
        magnitude1 = np.abs(fft_img1)
        magnitude2 = np.abs(fft_img2)
        phase1 = np.angle(fft_img1)
        phase2 = np.angle(fft_img2)
