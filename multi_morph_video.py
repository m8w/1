...
class MultiMorphApp:
            ...
            for frame in range(total_frames_per_transition):
                ...
                for technique, weight in selected_techniques.items():
                    if technique == "delaunay":
                        morphed = self.delaunay_morph(images[i], images[i+1], alpha)
                    elif technique == "crossdissolve":
                        morphed = self.cross_dissolve(images[i], images[i+1], alpha)
                    elif technique == "opticalflow":
                        morphed = self.optical_flow_morph(images[i], images[i+1], alpha)
                    elif technique == "gridwarp":
                        morphed = self.grid_warp(images[i], images[i+1], alpha)
                    elif technique == "frequency":
                        morphed = self.frequency_domain_morph(images[i], images[i+1], alpha)
    ...
    def grid_warp(self, img1, img2, alpha):
        ...
        for channel in range(2):  # x and y channels
            ...
            spline = RectBivariateSpline(y_points, x_points, 
                                         np.reshape(grid_points[:, channel] - alpha * offsets[:, channel], 
                                                   (len(y_points), len(x_points))))
            interp_vals = spline(y_full, x_full)
            if channel == 0:
                map_x1 = interp_vals
            else:
                map_y1 = interp_vals
            ...
            spline = RectBivariateSpline(y_points, x_points, 
                                         np.reshape(grid_points[:, channel] + (1-alpha) * offsets[:, channel], 
                                                   (len(y_points), len(x_points))))
            interp_vals = spline(y_full, x_full)
            if channel == 0:
                map_x2 = interp_vals
    ...
    def frequency_domain_morph(self, img1, img2, alpha):
        ...
        morph_intensity = np.real(ifft2(ifftshift(morph_fft))).astype(np.uint8)
        ...
        # Create the morphed HSV image using the frequency-morphed intensity
